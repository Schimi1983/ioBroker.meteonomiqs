...
class WetterComAdapter extends utils.Adapter {
    ...
    async onReady() {
        ...
        for (const entry of times) {
            ...
            // configured.
            const shifted = (0, helpers_1.shiftMinutes)(minutes, offset);
            planned.push(`${(0, helpers_1.pad)(Math.floor(shifted / 60))}:${(0, helpers_1.pad)(shifted % 60)} (tier ${entry.tier})`);
            const handle = (0, scheduler_1.scheduleDaily)(this, Math.floor(shifted / 60), shifted % 60, () => {
     ...
     */
    async resolveScheduleOffset() {
        // Explicitly widened: adapter-core types `namespace` as the template
        // literal `${string}.${number}`, so the inferred type would reject the
        // plain string coming from the UUID.
        ...
        return (0, helpers_1.scheduleOffsetMinutes)(seed, SCHEDULE_SPREAD_MINUTES);
    }
    /**
    ...
    fieldName(field, prefix) {
        return (0, i18n_1.prefixed)(prefix, (0, i18n_1.label)(field.name, field.nameDe));
    }
    /**
     ...
     */
    async renderFields(basePath, prefix, fields, source, ctx, buffer) {
        const active = fields.filter((f) => this.groupEnabled(f.group));
        for (const field of active) {
            await this.ensureState(`${basePath}.${field.id}`, (0, i18n_1.prefixed)(prefix, (0, i18n_1.label)(field.name, field.nameDe)), field.type, field.role, field.unit);
        }
        for (const field of active) {
            let raw;
            try {
    ...
    async ensureInfoStates() {
        const s = (id, en, de, type, role, unit, write = false) => this.ensureState(`info.${id}`, (0, i18n_1.label)(en, de), type, role, unit, write);
        ...
        if (this.config.enableJson) {
            await this.ensureState('forecast_json', (0, i18n_1.label)('Forecast (JSON)'), 'string', 'json');
            await this.ensureState('hourly_json', (0, i18n_1.label)('Hourly values (JSON)'), 'string', 'json');
        }
    }
    // ------------------------------------------------------------------- budget
    ...
    async processForecast(data, lang, forecastDays) {
        ...
        for (let index = 0; index < maxDays; index++) {
            ...
            const buffer = [];
            await this.ensureChannel(dayPath, (0, i18n_1.dayLabel)(index));
            await this.renderFields(dayPath, (0, i18n_1.dayPrefix)(index), fields_1.DAY_FIELDS, day, ctx, buffer);
            if (this.config.enableAstro && day.astronomy) {
                const astroPath = `${dayPath}.astro`;
                await this.ensureChannel(astroPath, (0, i18n_1.label)('Sun & moon'));
                await this.renderFields(astroPath, (0, i18n_1.dayPrefix)(index), fields_1.ASTRO_FIELDS, day.astronomy, ctx, buffer);
            }
            if (this.config.enableSpaces) {
                const spaceDay = spacesByDate.get(dayKeyIso);
                if (spaceDay) {
                    const spacesPath = `${dayPath}.spaces`;
                    await this.ensureChannel(spacesPath, (0, i18n_1.label)('Day sections'));
                    for (const segment of fields_1.SPACE_SEGMENTS) {
                        ...
                        await this.ensureChannel(segmentPath, (0, i18n_1.segmentLabel)(segment));
                        await this.renderFields(segmentPath, (0, i18n_1.daySegmentPrefix)(index, segment), fields_1.SPACE_FIELDS, segmentData, ctx, buffer);
                    }
                }
                else {
            ...
            if (this.config.enableHourly && index < hourlyDays) {
                ...
                await this.ensureChannel(hourlyPath, (0, i18n_1.label)('Hourly'));
                ...
                for (const hour of hours) {
                    ...
                    await this.ensureChannel(hourPath, (0, i18n_1.hourLabel)(hourText));
                    await this.renderFields(hourPath, (0, i18n_1.dayHourPrefix)(index, hourText), fields_1.HOUR_FIELDS, hour, ctx, buffer);
                ...
                for (let h = 0; h < 24; h++) {
                    const hourText = (0, helpers_1.pad)(h);
    ...
    async updateCurrent(reason) {
        ...
        try {
            const base = 'current';
            await this.ensureChannel(base, (0, i18n_1.label)('Current hour'));
            const hourFields = fields_1.HOUR_FIELDS.filter((f) => this.groupEnabled(f.group));
            for (const field of hourFields) {
                await this.ensureState(`${base}.${field.id}`, this.fieldName(field, i18n_1.NOW_PREFIX), field.type, field.role, field.unit);
            }
            for (const extra of fields_1.CURRENT_EXTRA) {
                if (extra.astro && !this.config.enableAstro) {
                    continue;
                }
                await this.ensureState(`${base}.${extra.id}`, this.fieldName(extra, i18n_1.NOW_PREFIX), extra.type, extra.role, extra.unit);
            }
            await this.ensureState(`${base}.source`, (0, i18n_1.prefixed)(i18n_1.NOW_PREFIX, (0, i18n_1.label)('Source state')), 'string', 'text');
            await this.ensureState(`${base}.updated`, (0, i18n_1.prefixed)(i18n_1.NOW_PREFIX, (0, i18n_1.label)('Last copied')), 'string', 'text');
