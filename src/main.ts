...
class WetterComAdapter extends utils.Adapter {
  ...
  private async onReady(): Promise<void> {
    ...
    if (times.length === 0) {
      this.log.error(
        "No valid update times configured. Please check the instance settings.",
      );
      return;
    }
    ...
    if (this.config.enableCurrent) {
      this.timers.push(
        scheduleHourly(this, 1, () => void this.updateCurrent("hourly")),
      );
    }
  ...
  private onStateChange(
    ...
    if (id.endsWith("info.force_update")) {
      ...
      this.log.info(
        "Manual update triggered (bypasses the cooldown, the monthly limit still applies).",
      );
  ...
  private parseUpdateTimes(): UpdateTime[] {
    ...
    for (const entry of raw) {
      ...
      if (minutes === null) {
        this.log.warn(
          `Ignoring invalid update time "${String(entry?.time)}" (expected HH:MM).`,
        );
        continue;
      }
  ...
  private get maxTier(): number {
    return this.parseUpdateTimes().reduce(
      (max, entry) => Math.max(max, entry.tier),
      1,
    );
  }
  ...
  private validateSchedule(times: UpdateTime[]): void {
    ...
    if (limit > 0 && worstCase > limit) {
      ...
      this.log.info(
        `Budget: ${perDay} fetches/day = at most ${worstCase} calls/month out of ${limit} (${limit - worstCase} spare).`,
      );
    }
  }
  ...
  private async resolveLocation(): Promise<{
    ...
    if (!this.config.useSystemLocation) {
      ...
      if (!isFinite(lat) || !isFinite(lon) || (lat === 0 && lon === 0)) {
        this.log.error(
          "Manual coordinates are missing or invalid. Please check the instance settings.",
        );
        return null;
      }
    ...
    if (!isFinite(lat) || !isFinite(lon)) {
      this.log.error(
        "No location configured in the ioBroker system settings (Settings → System → Location).",
      );
      return null;
    }
    return { lat: lat.toFixed(3), lon: lon.toFixed(3), lang };
  ...
  private fieldName(
    field: { name: string; nameDe: string },
    prefix: string,
  ...
  private async renderFields(
    ...
    for (const field of active) {
      ...
      buffer.push(
        this.setStateChangedAsync(`${basePath}.${field.id}`, {
  ...
  private async ensureInfoStates(): Promise<void> {
    const s = (
      ...
      this.ensureState(`info.${id}`, { en, de }, type, role, unit, write);
  ...
  private async rollPeriods(): Promise<void> {
    ...
    if (String(storedMonth?.val ?? "") !== month) {
      if (storedMonth?.val) {
        this.log.info(
          `New billing month detected (${String(storedMonth.val)} → ${month}). Monthly counter reset.`,
        );
      }
  ...
  private async checkBudget(
    ...
    if (limit > 0 && usage >= limit) {
      this.log.warn(
        `Monthly limit reached (${usage}/${limit}). Paused until the 1st of next month.`,
      );
    ...
    if (cooldown > 0 && lastTs > 0) {
      ...
      if (hours < cooldown) {
        this.log.debug(
          `Fetch skipped (cooldown). Last fetch ${hours.toFixed(1)} h ago, minimum ${cooldown} h.`,
        );
        return false;
      }
    }
    ...
    if (!verdict.allowed) {
      if (verdict.mode === "saving") {
        this.log.warn(
          `Saving mode: "${label}" skipped — the remaining budget no longer carries ${effectiveTier} fetches/day until the end of the month (${usage}/${limit}).`,
        );
      } else if (verdict.mode === "emergency") {
        this.log.warn(
          `Emergency mode: "${label}" skipped, budget critical (${usage}/${limit}). Fetching on even days only.`,
        );
      } else {
        this.log.warn(
          `"${label}" skipped, monthly limit reached (${usage}/${limit}).`,
        );
      }
    }
  ...
  private async fetchForecast(
    ...
    if (this.isFetching || this.unloaded) {
      this.log.debug(
        "Fetch already running or adapter shutting down — skipped.",
      );
      return;
    }
    ...
    try {
      ...
      if (this.authFailed && (reason === "scheduled" || reason === "startup")) {
        this.log.warn(
          "Fetch skipped: the API key was rejected previously. Please correct it in the instance settings.",
        );
        return;
      }
      ...
      if (apiKey.length < 10) {
        this.log.error(
          "No valid API key configured. Get one at https://www.meteonomiqs.com and enter it in the instance settings.",
        );
        await this.setStateAsync("info.status", {
          val: "no API key",
          ack: true,
      ...
      const forecastDays = Math.max(
        1,
        Math.min(
          Math.round(Number(this.config.forecastDays) || 7),
          MAX_FORECAST_DAYS,
        ),
      );
      const url = `${BASE_URL}/forecast/${location.lat}/${location.lon}`;
      this.log.info(
        `Fetching forecast (${label || reason}) for ${location.lat}/${location.lon}, ${forecastDays} days.`,
      );

      const maxRetries = Math.max(
        0,
        Math.round(Number(this.config.maxRetries) || 0),
      );
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        const mayRetry =
          ...
          (await this.getNumber(
            "info.requests_left",
            Number(this.config.monthlyLimit) || 0,
          )) > reserve;
        ...
        if (result.status === 429) {
          this.log.error(
            "API quota exhausted (HTTP 429). Setting the local counter to the configured limit.",
          );
        ...
        if (result.status === 401 || result.status === 403) {
          ...
          this.log.error(
            `API key rejected (HTTP ${result.status}). Automatic fetches are paused until the key is changed.`,
          );
          await this.setStateAsync("info.status", {
  ...
  private async processForecast(
    ...
    await Promise.all([
      ...
      this.setStateChangedAsync("info.elevation", {
        val: round(extractValue(data.location?.coordinates?.elevation), 0),
        ack: true,
      }),
    ]);
    ...
    for (const hour of data.hourly ?? []) {
      const parts = localParts(
        hour.from ?? hour.dateWithTimezone ?? hour.date,
        tz,
      );
    ...
    const hourlyDays = Math.max(
      0,
      Math.min(Math.round(Number(this.config.hourlyDays) || 0), maxDays),
    );
    ...
    for (let index = 0; index < maxDays; index++) {
      ...
      if (this.config.enableSpaces) {
        ...
        if (spaceDay) {
          ...
          this.log.debug(
            `No day sections for ${dayKeyIso} in the API response.`,
          );
        }
      }
      ...
      if (this.config.enableHourly && index < hourlyDays) {
        ...
        for (let h = 0; h < 24; h++) {
          ...
          if (this.ensured.has(validId)) {
            buffer.push(
              this.setStateChangedAsync(validId, { val: false, ack: true }),
            );
          }
        }
      }
  ...
  private async updateCurrent(reason: string): Promise<void> {
    ...
    if (!this.config.enableHourly || Number(this.config.hourlyDays) < 1) {
      this.log.warn(
        'The "current" folder needs hourly values for at least one day. Please enable them in the instance settings.',
      );
      return;
    }
    ...
    try {
      ...
      const hourlyDays = Math.max(
        1,
        Math.round(Number(this.config.hourlyDays) || 1),
      );
      ...
      const buffer: Promise<unknown>[] = [];
      const sourceHour =
        dayIndex >= 0 ? `day_${dayIndex}.hourly.${now.hour}` : "";

      if (!sourceHour || !(await this.getObjectAsync(`${sourceHour}.time`))) {
        buffer.push(
          this.setStateChangedAsync(`${base}.valid`, { val: false, ack: true }),
        );
        await Promise.all(buffer);
        this.log.debug(
          `current: no hourly data available for ${now.date} ${now.hour}:00.`,
        );
        return;
      }
      ...
      buffer.push(
        this.setStateChangedAsync(`${base}.source`, {
      ...
      await Promise.all(buffer);
      this.log.debug(
        `current updated (${reason}): ${now.date} ${now.hour}:00 from day_${dayIndex}.`,
      );
    } catch (e) {
      this.log.error(
        `current could not be updated: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
