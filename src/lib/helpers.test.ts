...
describe("helpers", () => {
  describe("extractValue()", () => {
    ...
    it("unwraps the nested API value objects", () => {
      ...
      expect(extractValue({ min: 1, max: 8, avg: 4 })).to.equal(
        4,
        "avg wins over min/max",
      );
      expect(extractValue({ value: null, avg: 3 })).to.equal(
        3,
  ...
  describe("parseApiDate()", () => {
    it("reads a bare YYYY-MM-DD as local midnight, not UTC", () => {
      ...
      expect(date!.getDate()).to.equal(
        6,
        "must not roll back a day west of UTC",
      );
    });
  ...
  describe("iconUrl()", () => {
    it("prefers the file name delivered by the API", () => {
      expect(iconUrl({ state: 63, icon: "d_e_63.svg" }, ICONS)).to.equal(
        `${ICONS}/d_e_63.svg`,
      );
      expect(iconUrl({ state: 0, icon: "n_0.svg" }, ICONS)).to.equal(
        `${ICONS}/n_0.svg`,
        "night icons must survive",
      );
    });
  ...
  describe("cloudsPercent()", () => {
    ...
    it("converts hourly oktas into percent", () => {
      expect(
        cloudsPercent({ low: 0, middle: 37.5, high: 50, eights: 6 }),
      ).to.equal(75);
  ...
  describe("isoTime() and dayLength()", () => {
    ...
    it("computes the day length", () => {
      expect(
        dayLength("2026-08-06T06:00:00+02:00", "2026-08-06T20:30:00+02:00"),
      ).to.equal(14.5);
  ...
  describe("budgetAllows()", () => {
    ...
    it("never exceeds the limit, whatever the starting point", () => {
      for (const start of [0, 10, 20, 40, 70, 95]) {
        ...
        for (let day = 1; day <= 31; day++) {
          for (const tier of [1, 2, 3]) {
            if (
              budgetAllows(usage, limit, tier, new Date(2026, 7, day, 12))
                .allowed
            ) {
              usage++;
            }
    ...
    it("drops the lowest priority first", () => {
      ...
      expect(budgetAllows(40, limit, 3, now).allowed).to.equal(
        false,
        "tier 3 goes first",
      );
      expect(budgetAllows(40, limit, 2, now).allowed).to.equal(
        true,
        "tier 2 still fits",
      );
      expect(budgetAllows(40, limit, 1, now).allowed).to.equal(
        true,
        "tier 1 survives longest",
      );
    });

    it("stops everything once the hard limit is reached", () => {
      const now = new Date(2026, 7, 8, 12);
      expect(budgetAllows(100, limit, 1, now)).to.deep.equal({
        allowed: false,
        mode: "limit",
      });
    ...
    it('treats a limit of 0 as "no limit configured"', () => {
      expect(
        budgetAllows(9999, 0, 3, new Date(2026, 7, 6, 12)).allowed,
      ).to.equal(true);
    });

    it("falls back to every other day instead of stopping completely", () => {
      expect(
        budgetAllows(95, limit, 1, new Date(2026, 7, 7, 12)).allowed,
      ).to.equal(false, "odd day");
      expect(
        budgetAllows(95, limit, 1, new Date(2026, 7, 8, 12)).allowed,
      ).to.equal(true, "even day");
    });
  });
});
