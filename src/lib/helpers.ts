...
import type { ApiNumber, ApiWeather } from "./types";
...
export function iconUrl(
  weather: ApiWeather | null | undefined,
  iconBase: string,
  isNight = false,
): string {
  const raw =
    weather && typeof weather.icon === "string" ? weather.icon.trim() : "";
  const state =
    weather && weather.state !== undefined && weather.state !== null
      ? weather.state
      : 999;
