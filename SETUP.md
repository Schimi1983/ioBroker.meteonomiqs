...
An drei Stellen steht `__GITHUB_USER__`. Einmal ersetzen:

```bash
cd ioBroker.wetter-com
# macOS:  sed -i '' 's/__GITHUB_USER__/deinuser/g' package.json io-package.json README.md
sed -i 's/__GITHUB_USER__/deinuser/g' package.json io-package.json README.md
grep -rn __GITHUB_USER__ . || echo "alle ersetzt"
```
